<?php 
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if(isset($_POST['order_info'])) {
            ...
            $customer_id 		= htmlentities($_POST['customer_id'], ENT_QUOTES, 'UTF-8');
            $reserve_id 		= htmlentities(@$_POST['reserve_id'], ENT_QUOTES, 'UTF-8');
			
			if($name != "" && $addr != "" && $email != "" && $phone != "" && $food != "" && $price != "") {
				
				$insert = $db->query("INSERT INTO basket(customer_id, reserve_id, customer_name, contact_number, address, email, total, status, date_made) VALUES('".$customer_id."','".$reserve_id."', '".$name."', '".$phone."', '".$addr."', '".$email."', '".$price."', 'pending', NOW())");
